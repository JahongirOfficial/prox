...
import { AlertCircle, Search, Users, TrendingUp } from 'lucide-react'
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex gap-2 text-xs">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-800/50 rounded-lg border border-slate-700/50">
            ...
            <span className="font-medium text-emerald-400">{stats.avgProgress}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative max-w-xs">
        ...
        <input
          ...
          className="w-full pl-8 pr-3 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:border-blue-500/50"
        />
      </div>
      ...
      {filteredStudents.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-slate-500 text-sm">{searchTerm ? 'Topilmadi' : 'O\'quvchilar yo\'q'}</p>
        </div>
      ) : (
          ...
          {filteredStudents.map((student) => {
            ...
            return (
              <div
                ...
                className={`flex items-center gap-2.5 px-3 py-2 bg-slate-800/30 hover:bg-slate-800/50 border border-slate-700/30 rounded-lg cursor-pointer transition ${student.is_blocked ? 'opacity-50' : ''}`}
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${isDebtor ? 'bg-red-500/15' : 'bg-emerald-500/15'}`}>
                  <span className={`font-semibold text-xs ${isDebtor ? 'text-red-400' : 'text-emerald-400'}`}>
                    {getInitials(student.name)}
                  </span>
                </div>
                <span className="flex-1 font-medium text-white text-sm truncate">{student.name || 'Nomsiz'}</span>
                <span className={`text-xs font-semibold ${isDebtor ? 'text-red-400' : 'text-emerald-400'}`}>{progress}%</span>
              </div>
            )
          })}
        </div>
