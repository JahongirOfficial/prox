...
function buildEmbedHTML(url: string): string {
  ...
  const ytWatch = u.match(
    /^https?:\/\/(?:www\.)?youtube\.com\/watch\?v=([\w-]{6,})/i,
  );
  if (ytWatch)
    return `<iframe src="https://www.youtube.com/embed/${ytWatch[1]}" allowfullscreen></iframe>`;
  const ytShort = u.match(/^https?:\/\/(?:www\.)?youtu\.be\/([\w-]{6,})/i);
  if (ytShort)
    return `<iframe src="https://www.youtube.com/embed/${ytShort[1]}" allowfullscreen></iframe>`;
  // Vimeo
  const vimeo = u.match(/^https?:\/\/vimeo\.com\/(\d+)/i);
  if (vimeo)
    return `<iframe src="https://player.vimeo.com/video/${vimeo[1]}" allowfullscreen></iframe>`;
  // Direct mp4
  if (/\.mp4($|\?)/i.test(u))
...
export default function Learning() {
  ...
  const getCurrentLessonIndex = () => {
    if (
      !selectedModuleId ||
      !selectedLesson ||
      !lessonsByModule[selectedModuleId]
    ) {
      return -1;
    }
  ...
  return (
    <div className="flex w-full min-h-screen bg-background text-foreground">
      {/* Sidebar: DESKTOP */}
      <aside className="hidden md:flex flex-col w-64 h-full bg-slate-900 border-r border-slate-700 justify-between fixed top-0 left-0 bottom-0 z-30">
        {/* Menu Content */}
                          ...
                          {loadingLessons[modId] ? (
                            ...
                            lessonsByModule[modId].map((lesson) => {
                              ...
                              return (
                                ...
                                >
                                  <PlayCircle
                                    className={`w-4 h-4 ${isActive ? "text-cyan-300" : "text-gray-400"}`}
                                  />
            ...
            {loadingLessons[selectedModuleId] ? (
              ...
              <>
                <h2 className="text-2xl font-bold mb-2 text-primary">
                  {selectedLesson ? selectedLesson.title : "Dars nomi"}
                </h2>
                <p className="text-base text-muted-foreground">
                  {selectedLesson && selectedLesson.description
                    ? selectedLesson.description
                    : "Dars haqida ma'lumotlar bu yerda ko'rsatiladi."}
                </p>
              </>
            )}
          </div>
